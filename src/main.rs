...
static ENV_PREFIX: &str = "kvim-envs";
...
fn env_appname(name: &str) -> String {
    format!("{}/{}", ENV_PREFIX, name)
}

fn env_config_dir(name: &str) -> PathBuf {
    xdg_config_home().join(ENV_PREFIX).join(name)
}

fn env_data_dir(name: &str) -> PathBuf {
...
fn env_cache_dir(name: &str) -> PathBuf {
...
fn cmd_env_create(name: &str, from: Option<&str>, branch: Option<&str>) -> Result<PathBuf, String> {
...
fn cmd_env_fork(source: &str, name: &str) -> Result<PathBuf, String> {
...
fn cmd_env_delete(name: &str) -> Result<(), String> {
    validate_env_name(name).map_err(|e| format!("Invalid env name: {}", e))?;
    ...
    if let Some(Commands::Env { action }) = &cli.command {
        let result = match action {
            EnvAction::Create { name, from, branch } => {
                cmd_env_create(name, from.as_deref(), branch.as_deref()).map(|_| ())
            }
            EnvAction::Fork { source, name } => {
                cmd_env_fork(source, name).map(|_| ())
            }
            EnvAction::List => {
                cmd_env_list();
                Ok(())
            }
            EnvAction::Delete { name } => cmd_env_delete(name),
        };
        if let Err(e) = result {
            eprintln!("{}", e);
            std::process::exit(1);
...
mod tests {
    ...
    // Helper: set XDG env vars to point at a temp dir, returns the TempDir (keeps it alive)
    ...
    // These tests must run serially since they mutate XDG env vars.
    ...
    fn test_env_delete_cleans_all_xdg_dirs() {
        ...
        // Simulate data/state/cache dirs existing
    ...
    fn test_env_fork_copies_all_dirs() {
        ...
        // Create source env with config + data + state + cache
